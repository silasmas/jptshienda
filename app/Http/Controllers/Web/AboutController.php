<?php

namespace App\Http\Controllers\Web;

use GuzzleHttp\Client;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\App;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Session;
use Illuminate\Support\Facades\Redirect;
use GuzzleHttp\Exception\ClientException;

/**
 * @author Xanders
 * @see https://www.linkedin.com/in/xanders-samoth-b2770737/
 */
class AboutController extends Controller
{
    public static $client;

    public function __construct()
    {
        // Client used for accessing API | Use authorization key
        $this::$client = new Client();

        // $this->middleware('auth');
    }

    // ==================================== HTTP GET METHODS ====================================
    /**
     * GET: Home page
     *
     * @return \Illuminate\View\View
     */
    public function index()
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];
        // Search group by name API URL
        $url_search_group = '/api/group/search/fonc';
        // Search status by name API URL
        $url_search_status = '/api/status/search/desact';
        // Create status API URL
        $url_status = '/api/status';
        // Select all about subjects API URL
        $url_about_subject = '/api/about_subject';

        try {
            // Search status by name API response
            $response_search_status = $this::$client->request('GET', $url_search_status, [
                'headers' => $headers,
                'verify'  => false
            ]);
            $search_status = json_decode($response_search_status->getBody(), false);

            if ($search_status->data != null) {
                foreach ($search_status->data as $status):
                    if ($status->status_name == 'Désactivé') {
                        try {
                            // Select all about subjects API response
                            $response_about_subject = $this::$client->request('GET', $url_about_subject, [
                                'headers' => $headers,
                                'verify'  => false
                            ]);
                            $about_subjects = json_decode($response_about_subject->getBody(), false);

                            return view('about', [
                                'about_subjects' => $about_subjects,
                                'status_id' => $status->id,
                            ]);

                        } catch (ClientException $e) {
                            // If API returns some error, display the message in the page
                            return view('about', [
                                'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                            ]);
                        }
                    }
                endforeach;

            } else {
                try {
                    // Search group by name API response
                    $response_search_group = $this::$client->request('GET', $url_search_group, [
                        'headers' => $headers,
                        'verify'  => false
                    ]);
                    $search_group = json_decode($response_search_group->getBody(), false);

                    foreach ($search_group->data as $group):
                        if ($group->group_name == 'Fonctionnement') {
                            try {
                                // Create status API response
                                $response_status = $this::$client->request('POST', $url_status, [
                                    'headers' => $headers,
                                    'form_params' => [
                                        'status_name' => 'Désactivé',
                                        'status_description' => 'Visibilité restreinte dans la plateforme ; et donc, impossibilité de faire des opérations.',
                                        'group_id' => $group->id,
                                    ],
                                    'verify'  => false
                                ]);
                                $status = json_decode($response_status->getBody(), false);
                                // Select all about subjects API response
                                $response_about_subject = $this::$client->request('GET', $url_about_subject, [
                                    'headers' => $headers,
                                    'verify'  => false
                                ]);
                                $about_subjects = json_decode($response_about_subject->getBody(), false);

                                return view('about', [
                                    'about_subjects' => $about_subjects,
                                    'status_id' => $status->data->id,
                                ]);

                            } catch (ClientException $e) {
                                // If API returns some error, display the message in the page
                                return view('about', [
                                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                                ]);
                            }
                        }                    
                    endforeach;

                } catch (ClientException $e) {
                    // If API returns some error, display the message in the page
                    return view('about', [
                        'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                    ]);
                }
            }

        } catch (ClientException $e) {
            // If API returns some error, display the message in the page
            return view('about', [
                'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
            ]);
        }
    }

    /**
     * GET: Home "entity" page
     *
     * @return \Illuminate\View\View
     */
    public function indexEntity($entity)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];

        if ($entity == 'about_title') {
            return view('about', [
                'entity' => $entity
            ]);
        }

        if ($entity == 'about_content') {
            return view('about', [
                'entity' => $entity
            ]);
        }

    }

    /**
     * GET: Data details
     *
     * @param  $data
     * @param  $id
     * @return \Illuminate\View\View
     */
    public function show($id)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];
        // Select about subject by ID API URL
        $url = '/api/about_subject/' . $id;

        try {
            // Select about subject by ID API response
            $response = $this::$client->request('GET', $url, [
                'headers' => $headers,
                'verify'  => false
            ]);
            $about_subject = json_decode($response->getBody(), false);

            return view('about', [
                'about_subject' => $about_subject
            ]);

        } catch (ClientException $e) {
            // If API returns some error, display the message in the page
            return view('about', [
                'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
            ]);
        }
    }

    /**
     * GET: Delete data
     *
     * @param  $data
     * @param  $id
     * @return \Illuminate\View\View
     */
    public function delete($data, $id)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];
        if ($data == 'role') {
            // Delete role API URL
            $url = '/api/role/' . $id;

            try {
                // Delete role API response
                $this::$client->request('DELETE', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);

                return Redirect::route('miscellaneous.home', [
                    'deleted_message' => 'done'
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                ]);
            }
        }

        if ($data == 'group') {
            // Delete group API URL
            $url = '/api/group/' . $id;

            try {
                // Delete group API response
                $this::$client->request('DELETE', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);

                return Redirect::route('miscellaneous.home', [
                    'deleted_message' => 'done'
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                ]);
            }
        }

        if ($data == 'service') {
            // Delete service API URL
            $url = '/api/service/' . $id;

            try {
                // Delete service API response
                $this::$client->request('DELETE', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);

                return Redirect::route('miscellaneous.home', [
                    'deleted_message' => 'done'
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
                ]);
            }
        }
    }

    // ==================================== HTTP POST METHODS ====================================
    /**
     * POST: Register a new data
     *
     * @param \Illuminate\Http\Request  $request
     * @param  $data
     * @return \Illuminate\View\View
     */
    public function store(Request $request)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];
        // Get inputs
        $inputs = [
            // About subject
            'subject' => $request->register_subject,
            'subject_description' => $request->register_description,
            'status_id' => $request->status_id,
            // Icon
            'icon_name' => $request->register_icon
        ];
        // Select all about subjects or Create new about subject API URL
        $url_about_subject = '/api/about_subject';
        // Create new icon API URL
        $url_icon = '/api/icon';

        try {
            // Create about subject API response
            $response_about_subject = $this::$client->request('POST', $url_about_subject, [
                'headers' => $headers,
                'form_params' => $inputs,
                'verify'  => false
            ]);
            $about_subject = json_decode($response_about_subject->getBody(), false);
            // Select all about subjects API response
            $response_about_subjects = $this::$client->request('GET', $url_about_subject, [
                'headers' => $headers,
                'verify'  => false
            ]);
            $about_subjects = json_decode($response_about_subjects->getBody(), false);
            // Create new icon API response
            $this::$client->request('POST', $url_icon, [
                'headers' => $headers,
                'form_params' => [
                    'icon_name' => $inputs['icon_name'],
                    'about_subject_id' => $about_subject->data->id,
                ],
                'verify'  => false
            ]);

            return view('about', [
                'created_message' => $about_subject->message,
                'status_id' => $inputs['status_id'],
                'about_subjects' => $about_subjects
            ]);

        } catch (ClientException $e) {
            // If API returns some error, display the message in the page
            // Select all about subjects API response
            $response = $this::$client->request('GET', $url_about_subject, [
                'headers' => $headers,
                'verify'  => false
            ]);
            $about_subjects = json_decode($response->getBody(), false);

            return view('about', [
                'inputs' => $inputs,
                'status_id' => $inputs['status_id'],
                'about_subjects' => $about_subjects,
                'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false)
            ]);
        }
    }

    /**
     * POST: Register a new data
     *
     * @param \Illuminate\Http\Request  $request
     * @param  $data
     * @return \Illuminate\View\View
     */
    public function storeEntity(Request $request, $data)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];
        // Select all roles or Create new role API URL
        $url_roles = '/api/role';
        // Select all groups or Create new group API URL
        $url_groups = '/api/group';
        // Select all services or Create new service API URL
        $url_services = '/api/service';

        if ($data == 'role') {
            // Get inputs
            $inputs = [
                'role_name' => $request->register_role_name,
                'role_description' => $request->register_role_description
            ];

            try {
                // Create role API response
                $response_new_role = $this::$client->request('POST', $url_roles, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $new_role = json_decode($response_new_role->getBody(), false);
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'created_message' => $new_role->message,
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the role page
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'inputs' => $inputs,
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);
            }
        }

        if ($data == 'group') {
            // Get inputs
            $inputs = [
                'group_name' => $request->register_group_name,
                'group_description' => $request->register_group_description
            ];

            try {
                // Create group API response
                $response_new_group = $this::$client->request('POST', $url_groups, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $new_group = json_decode($response_new_group->getBody(), false);
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'created_message' => $new_group->message,
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'inputs' => $inputs,
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);
            }
        }

        if ($data == 'service') {
            // Get inputs
            $inputs = [
                'service_name' => $request->register_service_name,
                'phone_operator' => $request->register_phone_operator
            ];
            // Select all services or Create new service API URL
            $url = '/api/service';

            try {
                // Create service API response
                $response_new_service = $this::$client->request('POST', $url, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $new_service = json_decode($response_new_service->getBody(), false);
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'created_message' => $new_service->message,
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                // Select all roles API response
                $response_roles = $this::$client->request('GET', $url_roles, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $roles = json_decode($response_roles->getBody(), false);
                // Select all groups API response
                $response_groups = $this::$client->request('GET', $url_groups, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $groups = json_decode($response_groups->getBody(), false);
                // Select all services API response
                $response_services = $this::$client->request('GET', $url_services, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $services = json_decode($response_services->getBody(), false);

                return view('miscellaneous', [
                    'inputs' => $inputs,
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'roles' => $roles,
                    'groups' => $groups,
                    'services' => $services
                ]);
            }
        }
    }

    /**
     * POST: Update data
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  $data
     * @param  $id
     * @return \Illuminate\View\View
     */
    public function update(Request $request, $data, $id)
    {
        // Get header informations
        $headers = [
            'Authorization' => 'Bearer uWNJB6EwpVQwSuL5oJ7S7JkSkLzdpt8M1Xrs1MZITE1bCEbjMhscv8ZX2sTiDBarCHcu1EeJSsSLZIlYjr6YCl7pLycfn2AAQmYm',
            'Accept' => 'application/json',
            'X-localization' => !empty(Session::get('locale')) ? Session::get('locale') : App::getLocale()
        ];

        if ($data == 'role') {
            // Get inputs
            $inputs = [
                'id' => $request->role_id,
                'role_name' => $request->register_role_name,
                'role_description' => $request->register_role_description
            ];
            // Select role or Update role API URL
            $url = '/api/role/' . $id;

            try {
                // Update role API response
                $response = $this::$client->request('PUT', $url, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $role = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'role' => $role,
                    'updated_message' => $role->message,
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                // Select role API response
                $response = $this::$client->request('GET', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $role = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'inputs' => $inputs,
                    'role' => $role
                ]);
            }
        }

        if ($data == 'group') {
            // Get inputs
            $inputs = [
                'id' => $request->group_id,
                'group_name' => $request->register_group_name,
                'group_description' => $request->register_group_description
            ];
            // Select group or Update group API URL
            $url = '/api/group/' . $id;

            try {
                // Update group API response
                $response = $this::$client->request('PUT', $url, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $group = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'group' => $group,
                    'updated_message' => $group->message,
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                // Select group API response
                $response = $this::$client->request('GET', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $group = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'inputs' => $inputs,
                    'group' => $group
                ]);
            }
        }

        if ($data == 'service') {
            // Get inputs
            $inputs = [
                'id' => $request->service_id,
                'service_name' => $request->register_service_name,
                'phone_operator' => $request->register_phone_operator
            ];
            // Select service or Update service API URL
            $url = '/api/service/' . $id;

            try {
                // Update service API response
                $response = $this::$client->request('PUT', $url, [
                    'headers' => $headers,
                    'form_params' => $inputs,
                    'verify'  => false
                ]);
                $service = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'service' => $service,
                    'updated_message' => $service->message,
                ]);

            } catch (ClientException $e) {
                // If API returns some error, display the message in the miscellaneous page
                // Select service API response
                $response = $this::$client->request('GET', $url, [
                    'headers' => $headers,
                    'verify'  => false
                ]);
                $service = json_decode($response->getBody(), false);

                return view('miscellaneous', [
                    'response_error' => json_decode($e->getResponse()->getBody()->getContents(), false),
                    'inputs' => $inputs,
                    'service' => $service
                ]);
            }
        }
    }
}
